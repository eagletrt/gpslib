...
int gps_interface_open_gpsd(gps_serial_port *port, const char *ip) {
  if (!ip)
    return -1;
  ...
  if (0 != gps_open(ip, "2947", &port->gpsd)) {
    printf("GPS Interface: Error opening fd\n");
    return -1;
  }

  gps_stream(&port->gpsd, WATCH_ENABLE | WATCH_RAW, NULL);

  port->fd = port->gpsd.gps_fd;
  port->open = 1;
  return 0;
}

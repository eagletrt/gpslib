...
void gps_parse_ubx_dop(gps_ubx_dop_t* data, uint8_t* buffer){
    ...
    data->iTOW = *(uint32_t*)buffer; buffer += 4;
    data->gDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
    data->pDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
    data->tDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
    data->vDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
    data->hDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
    data->nDOP = (*(uint16_t*)buffer)*0.01f; buffer += 2;
...
void gps_parse_ubx_pvt(gps_ubx_pvt_t* data, uint8_t* buffer){
    buffer += 4;    // align to start of payload
    data->iTOW      = *(uint32_t*)buffer; buffer += 4;
    data->year      = *(uint16_t*)buffer; buffer += 2;
    data->month     = *(uint8_t*)buffer; buffer += 1;
    data->day       = *(uint8_t*)buffer; buffer += 1;
    data->hour      = *(uint8_t*)buffer; buffer += 1;
    data->min       = *(uint8_t*)buffer; buffer += 1;
    data->sec       = *(uint8_t*)buffer; buffer += 1;
    data->valid     = *(uint8_t*)buffer; buffer += 1;
    data->tAcc      = *(uint32_t*)buffer; buffer += 4;
    data->nano      = *(int32_t*)buffer; buffer += 4;
    data->fixType   = *(uint8_t*)buffer; buffer += 1;
    data->flags     = *(uint8_t*)buffer; buffer += 1;
    data->flags2    = *(uint8_t*)buffer; buffer += 1;
    data->numSV     = *(uint8_t*)buffer; buffer += 1;
    data->lon       = *(int32_t*)buffer; buffer += 4;
    data->lat       = *(int32_t*)buffer; buffer += 4;
    data->height    = *(int32_t*)buffer; buffer += 4;
    data->hMSL      = *(int32_t*)buffer; buffer += 4;
    data->hAcc      = *(uint32_t*)buffer; buffer += 4;
    data->vAcc      = *(uint32_t*)buffer; buffer += 4;
    data->velN      = *(int32_t*)buffer; buffer += 4;
    data->velE      = *(int32_t*)buffer; buffer += 4;
    data->velD      = *(int32_t*)buffer; buffer += 4;
    data->gSpeed    = *(int32_t*)buffer; buffer += 4;
    data->headMot   = *(int32_t*)buffer; buffer += 4;
    data->sAcc      = *(uint32_t*)buffer; buffer += 4;
    data->headAcc   = *(uint32_t*)buffer; buffer += 4;
    data->pDOP      = *(uint16_t*)buffer; buffer += 2;
    buffer += 6;    // reserved
    data->headVeh   = *(int32_t*)buffer; buffer += 4;
    data->magDec    = *(int16_t*)buffer; buffer += 2;
...
void gps_parse_ubx_hpposecef(gps_ubx_hpposecef_t* data, uint8_t* buffer){
    buffer += 4; // align to start of payload
    data->version   = *(uint8_t*)buffer; buffer += 1;
    buffer += 3;// 3 reserved bytes
    data->iTOW      = *(uint32_t*)buffer; buffer += 4;
    data->ecefX     = *(int32_t*)buffer; buffer += 4;
    data->ecefY     = *(int32_t*)buffer; buffer += 4;
    data->ecefZ     = *(int32_t*)buffer; buffer += 4;
    data->ecefXHp   = *(int8_t*)buffer; buffer += 1;
    data->ecefYHp   = *(int8_t*)buffer; buffer += 1;
    data->ecefZHp   = *(int8_t*)buffer; buffer += 1;
    buffer += 1;    // reserved
...
void gps_parse_ubx_hpposllh(gps_ubx_hpposllh_t* data, uint8_t* buffer){
    ...
    data->height    = *(int32_t*)buffer; buffer += 4;
    data->hMSL      = *(int32_t*)buffer; buffer += 4;
    data->lonHp     = *(int8_t*)buffer; buffer += 1;
    data->latHp     = *(int8_t*)buffer; buffer += 1;
    data->heightHp  = *(int8_t*)buffer; buffer += 2;
    data->hMSLHp    = *(int8_t*)buffer; buffer += 2;
    data->hAcc      = *(uint32_t*)buffer; buffer += 4;
...
void gps_nmea_fields_gga(FILE* out){
    fprintf(out, "_timestamp,time,latitude,north_south,longitude,east_ovest,fix,satellites,horizontal_diluition_precision,fix_state,altitude,age_of_correction\n");
    fflush(out);
}

void gps_nmea_fields_vtg(FILE* out){
    fprintf(out, "_timestamp,course_over_ground_degrees,course_over_ground_degrees_magnetic,speed_kmh\n");
    fflush(out);
}

void gps_nmea_fields_gsa(FILE* out){
    fprintf(out, "_timestamp,mode,position_diluition_precision,horizontal_diluition_precision,vertical_diluition_precision\n");
    fflush(out);
}
...
void gps_ubx_fields_pvt(FILE* out){
    fprintf(out, "_timestamp,iTOW,year,month,day,hour,min,sec,valid,tAcc,nano,fixType,flags,flags2,numSV,lon,lat,height,hMSL,hAcc,vAcc,velN,velE,velD,gSpeed,headMot,sAcc,headAcc,pDOP,headVeh,magDec,magAcc\n");
    fflush(out);
}

void gps_ubx_fields_hpposecef(FILE* out){
    fprintf(out, "_timestamp,version,iTOW,ecefX,ecefY,ecefZ,ecefXHp,ecefYHp,ecefZHp,pAcc\n");
    fflush(out);
}

void gps_ubx_fields_hpposllh(FILE* out){
    fprintf(out, "_timestamp,version,iTOW,lon,lat,height,hMSL,lonHp,latHp,heightHp,hMSLHp,hAcc,vAcc\n");
    fflush(out);
}
...
void gps_nmea_value_to_file_vtg(FILE* out, gps_nmea_vtg_t* data){
    fprintf(out,
        ...
        "\n",
        data->_timestamp,
        data->course_over_ground_degrees,
...
void gps_nmea_value_to_file_gsa(FILE* out, gps_nmea_gsa_t* data){
    fprintf(out,
        ...
        data->_timestamp,
        data->mode,
        data->position_diluition_precision,
        data->horizontal_diluition_precision,
...
gps_parse_result_t gps_parse_buffer(gps_parsed_data_t* data, gps_protocol_and_message* match, const char* buffer, uint64_t timestamp){

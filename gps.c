...
int gps_ubx_check_checksum(const char *buffer) {
  ...
  if (gps_buffer_to_generic_ubx_message(&generic_msg, buffer) == -1) return -1;
  rfc1145_checksum(&ch_a, &ch_b, buffer, generic_msg._size + 4);
  if (ch_a == generic_msg._ck_a && ch_b == generic_msg._ck_b) return 0;
  return -1;
}
...
void gps_nmea_fields_gga(FILE *out) {
  fprintf(out,
          "timestamp,time,latitude,north_south,longitude,east_ovest,fix,"
...
void gps_nmea_fields_vtg(FILE *out) {
  fprintf(out,
          "timestamp,course_over_ground_degrees,course_over_ground_"
...
void gps_nmea_fields_gsa(FILE *out) {
  fprintf(out,
          "timestamp,mode,position_diluition_precision,horizontal_"
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_DOP_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_PVT_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_HPPOSECEF_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_HPPOSLLH_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_RELPOSNED_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
#define FIELD(byte_offset, original_type, struct_type, formatter, scale, \
              offset, unit, name)                                        \
  "," #name
  fprintf(out, "timestamp" GPS_UBX_VELNED_FIELDS "\n");
#undef FIELD
  fflush(out);
}
...
void gps_nmea_value_to_file_gga(FILE *out, gps_nmea_gga_t *data) {
  fprintf(out,
          ...
          ",%.9f"
          "\n",
          data->timestamp, data->time, data->latitude, data->north_south,
...
void gps_nmea_value_to_file_vtg(FILE *out, gps_nmea_vtg_t *data) {
  fprintf(out,
          ...
          ",%.9f"
          "\n",
          data->timestamp, data->course_over_ground_degrees,
...
void gps_nmea_value_to_file_gsa(FILE *out, gps_nmea_gsa_t *data) {
  fprintf(out,
          ...
          ",%.9f"
          "\n",
          data->timestamp, data->mode, data->position_diluition_precision,
...
gps_parse_result_t gps_parse_buffer(gps_parsed_data_t *data,
  ...
  if (match->protocol == GPS_PROTOCOL_TYPE_NMEA) {
    switch (match->message) {
      case GPS_NMEA_TYPE_GGA:
        data->gga.timestamp = timestamp;
        gps_parse_nmea_gga(&data->gga, buffer);
        break;
      case GPS_NMEA_TYPE_GSA:
        data->gsa.timestamp = timestamp;
        gps_parse_nmea_gsa(&data->gsa, buffer);
        break;
      case GPS_NMEA_TYPE_VTG:
        data->vtg.timestamp = timestamp;
    ...
    if (gps_ubx_check_checksum(buffer) != 0) return GPS_PARSE_RESULT_CHECKSUM;
    switch (match->message) {
      case GPS_UBX_TYPE_NAV_DOP:
        data->dop.timestamp = timestamp;
        gps_parse_ubx_dop(&data->dop, (uint8_t *)buffer);
        break;
      case GPS_UBX_TYPE_NAV_PVT:
        data->pvt.timestamp = timestamp;
        gps_parse_ubx_pvt(&data->pvt, (uint8_t *)buffer);
        break;
      case GPS_UBX_TYPE_NAV_HPPOSECEF:
        data->hpposecef.timestamp = timestamp;
        gps_parse_ubx_hpposecef(&data->hpposecef, (uint8_t *)buffer);
        break;
      case GPS_UBX_TYPE_NAV_HPPOSLLH:
        data->hpposllh.timestamp = timestamp;
        gps_parse_ubx_hpposllh(&data->hpposllh, (uint8_t *)buffer);
        break;
      case GPS_UBX_TYPE_NAV_RELPOSNED:
        data->relposned.timestamp = timestamp;
        gps_parse_ubx_relposned(&data->relposned, (uint8_t *)buffer);
        break;
      case GPS_UBX_TYPE_NAV_VELNED:
        data->velned.timestamp = timestamp;
...
void gps_to_file(gps_files_t *files, gps_parsed_data_t *data,
  ...
  switch (match->protocol) {
    case GPS_PROTOCOL_TYPE_NMEA:
      switch (match->message) {
        case GPS_NMEA_TYPE_GGA:
          gps_nmea_value_to_file_gga(files->nmea[GPS_NMEA_TYPE_GGA],
                                     &data->gga);
          break;
        case GPS_NMEA_TYPE_VTG:
          gps_nmea_value_to_file_vtg(files->nmea[GPS_NMEA_TYPE_VTG],
                                     &data->vtg);
          break;
        case GPS_NMEA_TYPE_GSA:
          gps_nmea_value_to_file_gsa(files->nmea[GPS_NMEA_TYPE_GSA],
                                     &data->gsa);
          break;
        default:
          break;
    ...
    case GPS_PROTOCOL_TYPE_UBX:
      switch (match->message) {
        case GPS_UBX_TYPE_NAV_DOP:
          gps_ubx_value_to_file_dop(files->ubx[GPS_UBX_TYPE_NAV_DOP],
                                    &data->dop);
          break;
        case GPS_UBX_TYPE_NAV_PVT:
          gps_ubx_value_to_file_pvt(files->ubx[GPS_UBX_TYPE_NAV_PVT],
                                    &data->pvt);
          break;
        case GPS_UBX_TYPE_NAV_HPPOSECEF:
          gps_ubx_value_to_file_hpposecef(
              files->ubx[GPS_UBX_TYPE_NAV_HPPOSECEF], &data->hpposecef);
          break;
        case GPS_UBX_TYPE_NAV_HPPOSLLH:
          ...
          break;
        case GPS_UBX_TYPE_NAV_RELPOSNED:
          gps_ubx_value_to_file_relposned(
              files->ubx[GPS_UBX_TYPE_NAV_RELPOSNED], &data->relposned);
          break;
        case GPS_UBX_TYPE_NAV_VELNED:
